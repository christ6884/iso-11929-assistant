...
const AnalyticalChart: React.FC<Omit<ChartPanelProps, 'results'> & { results: Results }> = ({ results, t, mode, calibrationFactorUnit }) => {
    ...
    return <>
        ...
        <div className="flex justify-center flex-wrap gap-x-4 gap-y-2 text-xs mt-4">
            <div className="flex items-center"><div className="w-3 h-3 rounded-full bg-blue-400 mr-1.5"></div>H₀ ({t('h0_legend')})</div>
            <div className="flex items-center"><div className="w-3 h-3 rounded-full bg-violet-400 mr-1.5"></div>H₁ ({t('h1_legend')})</div>
            {(mode === 'standard' || mode === 'spectrometry') && <div className="flex items-center"><div className="w-3 h-3 rounded-full bg-cyan-400/50 mr-1.5"></div>{t('measurement_distribution_legend')}</div>}
            {(mode === 'standard' || mode === 'spectrometry') && isEffectPresent && <div className="flex items-center"><div className="w-3 h-3 rounded-sm bg-cyan-400/70 mr-1.5"></div>{t('confidenceIntervalLegend')}</div>}
            <div className="flex items-center"><div className="w-3 h-3 rounded-sm bg-yellow-500/50 mr-1.5"></div>α ({t('alpha_risk_legend')})</div>
            <div className="flex items-center"><div className="w-3 h-3 rounded-sm bg-violet-500/50 mr-1.5"></div>β ({t('beta_risk_legend')})</div>
        </div>
        <p className="text-xs text-center text-gray-500 mt-3">{t('chartDescription')}</p>
    </>
}

const MonteCarloChart: React.FC<Omit<ChartPanelProps, 'results'> & { results: Results }> = ({ results, t, mode, calibrationFactorUnit }) => {
    ...
    return <>
        ...
        <div className="flex justify-center flex-wrap gap-x-3 gap-y-2 text-xs mt-4">
...
const ChartPanel: React.FC<ChartPanelProps> = ({ results, t, mode, calibrationFactorUnit }) => {
            ...
            <div className="flex items-center space-x-4">
                {results.calculationMethod === 'monteCarlo' && (
                    ...
                    >
                        ...
                        <span>{t('viewSimulationStats')}</span>
                    </button>
                )}
                <button
        ...
        <>
            <Card title={cardTitle}>
                <div className="bg-gray-900 p-2 rounded-lg">
                    {results.calculationMethod === 'monteCarlo' 
                        ? <MonteCarloChart results={results} t={t} mode={mode} calibrationFactorUnit={calibrationFactorUnit} />
                        : <AnalyticalChart results={results} t={t} mode={mode} calibrationFactorUnit={calibrationFactorUnit} />
                    }
                </div>
            </Card>
            <MonteCarloStatsModal 
            ...
            <ChartModal 
                ...
                t={t}
                mode={mode}
                calibrationFactorUnit={calibrationFactorUnit}
            />
        </>
    );
};
